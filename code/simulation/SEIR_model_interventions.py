...
def SEIR(start_org_date,
    ...
    ----------
        ...
        @param remove_fractions (List[float]): share of total removal for each social group (should sum to 1)
        @param remove_start_day (int): day when behavior removal begins
        @param total_removal_target (int): total proportion of individuals to remove across the whole population
    ...
    for t in np.arange(1, T, 1):
        ...
        # handle the removal since the remove_start_day
        if t == remove_day:
            num_remove_social_group = total_num_removal * remove_fractions # white, black, hispanic, asian, others=0
            print('removals', num_remove_social_group[:-1])
            print('population', Nk[:-1])
            print('proportion of removals', num_remove_social_group[:-1]/np.array(Nk[:-1]))
            for ig in range(ngroups):
                compartments_next_day[0, ig] -= int(num_remove_social_group[ig])
                # compartments_next_day[0, ig] -= num_remove_social_group[ig] * compartments_next_day[0, ig] / np.sum(compartments_next_day[:, ig])
                # compartments_next_day[1, ig] -= num_remove_social_group[ig] * compartments_next_day[1, ig] / np.sum(compartments_next_day[:, ig])
                # compartments_next_day[2, ig] -= num_remove_social_group[ig] * compartments_next_day[2, ig] / np.sum(compartments_next_day[:, ig])
                # compartments_next_day[3, ig] -= num_remove_social_group[ig] * compartments_next_day[3, ig] / np.sum(compartments_next_day[:, ig])
